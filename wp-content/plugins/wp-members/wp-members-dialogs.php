...
function wpmem_inc_registration_NEW( $toggle = 'new', $heading = '' )
{
	...
	$form = $form . '<input name="redirect_to" type="hidden" value="' . get_permalink( get_page_by_title( 'List' ) ) . '" />
...
function wpmem_login_form_NEW( $page, $arr ) 
{
	...
	$redirect_to = ( isset( $_REQUEST['redirect_to'] ) ) ? esc_url( $_REQUEST['redirect_to'] ) : get_permalink( get_page_by_title('List'));
